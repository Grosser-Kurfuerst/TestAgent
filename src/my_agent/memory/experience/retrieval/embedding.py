...
from my_agent.memory.experience.retrieval.text import experience_searchable_text
...
class EmbeddingRetriever:
    ...
    def fork(self) -> "EmbeddingRetriever":
        return EmbeddingRetriever(self.encoder, cache=self.cache)
        ...
        *,
        ...
        try:
            ...
            raise EmbeddingRetrievalError(
                f"formal embedding retrieval failed: {type(exc).__name__}: {exc}"
            ) from exc
    ...
    def retrieve_candidates(
        ...
        *,
        ...
        return flatten_per_tier_hits(self.retrieve_per_tier(
            query,
            store=store,
            project_key=project_key,
    ...
    def _build_index(
        ...
        *,
        ...
        cache_hit_count = 0
        for tier in ExperienceTier:
            memories = visible_memories_for_tier(
                repository,
                project_key=project_key,
                tier=tier,
            )
            ...
            if pending:
                ...
                if len(encoded) != len(pending):
                    raise EmbeddingRetrievalError(
                        "embedding backend returned the wrong document count"
                    )
...
class TransformersEmbeddingEncoder:
    ...
    def from_config(cls, config: AgentConfig) -> "TransformersEmbeddingEncoder":
        try:
            ...
            raise RuntimeError(
                "formal embedding retrieval requires the 'opd-embed' extra"
            ) from exc
    ...
    def _encode(self, texts: Sequence[str]) -> tuple[tuple[float, ...], ...]:
        ...
        return tuple(
            ...
            for row in normalized.float().cpu().tolist()
        )
...
def _single_vector(vectors: Sequence[Sequence[float]], label: str) -> tuple[float, ...]:
    if len(vectors) != 1:
        raise EmbeddingRetrievalError(
            f"embedding backend returned the wrong {label} count"
        )
